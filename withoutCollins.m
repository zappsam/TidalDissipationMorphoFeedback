...
P.kro=0.2;%0.5;%0.2;%0.2;%0.1; % minimum water depth [m]. 0.2 is good NEEDS TO BE SMALLER THAN hwSea_lim
P.DiffSmud=0.1; %coefficient for tidal dispersion [-]. DO NOT CHANGE
P.DiffSsand=1; %coefficient for tidal dispersion [-]. DO NOT CHANGE

%Sea level rise
P.RSLR=3/1000/365;  %from mm/yr to m/day (the time unit is the day!)
...
P.Cbr=0.73;
P.Cbed=0.038;%67;%38;%NaN;%wave bed friction if you use Jonswap (0.067 or 0.038)
P.wavefrictionCollins=0;

%Wind for sea waves
P.wind=6.9;%reference wind speed [m/s]
...
P.hwSwell_lim=0.2;%1; %limiter water depth for swell
...
P.hlimC=1;%limit to apply something... dont rember
...
P.fMFswell=0.1; %use 1 if you use the equvilanet wave height
...
%Time parameters
tmax=5000;%1250;% number of time steps
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%time series
numberserie=10000;%if you change this you will change the actual values in the time series, rememebr!
...
[N,M,dx,A,AW,Yb,Y1,Y2,Y3,zb,z,plyr,flyr1,flyr2,flyr3,flyrb1,flyrb2,flyrb3,x,y,msl,SPCLcell]=initializegeometry_3sedimentsbarrierbigfar(P);
%savegeometry_3sediments(N,M,dx,A,AW,Yb,plyr,zb,z,Y1,Y2,Y3,flyr1,flyr2,flyr3,flyrb1,flyrb2,flyrb3,x,y,msl,SPCLcell,S,'CLEAN_R3');
...
makevideo=0;
%v=VideoWriter('downslopebreaking_alongw_q','Motion JPEG AVI');
...
%if t>2000;P.RSLR=3/1000/365;end
...
ax1 = subplot(1,3,1);%s+1 %set(IM,'alphadata',~(A==0));s+1
...
IM=imagesc(y,x,SSC1*1000);axis equal;set(IM,'alphadata',~(A==0));set(gca,'YDir','normal');%colormap('jet');
...
if abs(checksum(1))>0.1;pause;end
%RfIO.FQsW_L
%fIO.FQsW_R
