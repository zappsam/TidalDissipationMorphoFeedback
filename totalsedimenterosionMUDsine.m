function [E,Etide]=totalsedimenterosionMUDsine(U,MANN,VEG,fTide,UR,Uwave_sea,Uwave,Tp_sea,Tp_swell,fMFswell,fMFsea,fMFriver,taucr,tcrgradeint,leveltauincrease,taucrVEG,me,h,lev,TrangeVEG,computeSeaWaves,computeSwellwave,computeRiver);
fUpeak=pi/2;
...
tauC(h<0.3)=min(1,tauC(h<0.3));%limiter for tay, since the smallest water depth kro = 5cm
E=E+1/(ncyc+1)*me.*(sqrt(1+(tauC./taucro).^2)-1);%E=E+1/(ncyc+1)*me.*taucr./taucro.*(sqrt(1+(tauC./taucr).^2)-1);
end
Etide=E;

%%Swell erosion
if computeSwellwave==1
%ko=1/1000;%0.1/1000;%0.1*3
...
E=E+me.*(sqrt(1+(tauWswell./taucro).^2)-1)*fMFswell.*fTide; %OCIO AI TOLTO QUIESTI fTide IN APRIL 18th 2018
end
