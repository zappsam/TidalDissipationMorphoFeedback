...
slope=0.4/1000;z=[0:N-1]*slope*dx;z=z'*ones(1,M)-3;%sloping   0.5
%slope=0.5/1000;z=[0:N-1]*slope*dx;z=z'*ones(1,M)-3;%sloping
%zin=5;z=zin*ones(N,M);%flat

slopebreak=70;
z=[1:N];
slope=0.4/1000;z(1:N-slopebreak)=[1:N-slopebreak]*slope*dx-3+5;
slope=1/1000;z(slopebreak+1:N)=+z(slopebreak)+[slopebreak+1:N]*slope*dx+5;
z=z'*ones(1,M);%sloping

%barriers
pos=65*dx/dx;bwidth=800/dx;
...
z(end-pos-bwidth:end-pos,:)=-3;%3;
%z(end-pos+1:end,:)=z(end-pos+1:end,:)+5;

%z(end-pos-10:end-pos+10,M/2-2:M/2+2)=10;
