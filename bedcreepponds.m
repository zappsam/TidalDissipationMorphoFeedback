function z=bedcreepponds(z,A,Active,Yreduction,crMUD,crMARSH,crbank,dx,dt,VEG,S,Qs,rbulk,alphaMUD,facQsbank);%,deltaUC,a_bankcreep);
...
%value=(D(p(a))+D(q(a)))/2 +(DQs(p(a))+DQs(q(a)))/2;%  +(DUc(p(a))+DUc(q(a)))/2.*max(VEG(p(a))==1,VEG(q(a))==1); 
