function [EmD,SSM,FLX,XXTide,XXRiver]=sedtran(numeric,d,A,SPCLcell,DoMUD,DiffS,h,ho,E,WS,dx,dt,rbulk,co,Ux,Uy,FLX,fTide,Ttide,URx,URy,UresX,UresY,periodic,computeriver,computetide,residualcurrents,kro,MUD,coMUD,tracksedimentfluxes,Qsmouthi,XX);
...
%Tide
% SSCp=SSM(p(a))./h(p(a))./fTide(p(a));SSCq=SSM(q(a))./h(q(a))./fTide(q(a));
% SSCp(SSCp>10)
SSCf=SSM./h./fTide;
%SSCf(SSCf>10^3 & h<0.1)=NaN;
SSCf(SSCf>10^2)=NaN;
%Q=Q+sum(DD.*(SSM(p(a))./h(p(a))./fTide(p(a))-SSM(q(a))./h(q(a))./fTide(q(a)))); %exit from that cell
Q=Q+nansum(DD.*(SSCf(p(a))-SSCf(q(a))));
%River
if computeriver==1;
if (k==N); QR=QR+sum( SSM(p(a))./h(p(a)).*qN(p(a)) ./fTide(p(a)) )*sign(k);end
if (k==-N);QR=QR+sum( SSM(p(a))./h(p(a)).*qmN(p(a))./fTide(p(a)) )*sign(k);end
if (k==1); QR=QR+sum( SSM(p(a))./h(p(a)).*q1(p(a)) ./fTide(p(a)) )*sign(k);end
if (k==-1);QR=QR+sum( SSM(p(a))./h(p(a)).*qm1(p(a))./fTide(p(a)) )*sign(k);end
end
...
else%do not calcualte the fluxes of all
XXTide=NaN;
XXRiver=NaN;
