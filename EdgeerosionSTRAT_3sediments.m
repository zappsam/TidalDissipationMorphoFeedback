function [deltaY1,deltaY2,deltaY3,Pedge,Y2OX]=Edgeerosion(P,z,aw,maxedgeheight,fox,dt,dx,MASK,A,fracY1,fracY2,fracY3,Y2OX);
...
for i=1:length(edger)
    ...
    %a=find(MASK(p)==1); %only choses the adjacent cells if they are mudlfat and if they are standard cells
 
    
 %pause(1)
    if a>0 %yes, there are adjacent cells   
            dz=([z(edger(i))-z(p(a))]);
            dz=mean(max(dz,0));%dz=min(max(dz,0));
