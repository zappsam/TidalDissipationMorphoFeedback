function [IO,fIO,maxdeltaz,maxup,PLT,MMM]=mainevolutionstep(A,AW,SPCLcell,PARAMS,x,dx,dt,zb,IO,fIO,Hsurge,angleSWELL,angleWIND,t)
...
%uncomment next 6 lines for dynamic tidal
%attenuation
[Trange]=gettidalrange(Ttide,Trange,Cb,Cv,dx,x,z-msl,A);
[Trange90]=gettidalrange(Ttide90,Trange90_o,Cb,Cv,dx,x,z-msl,A);
% 
% if isnan(mean(Trange90,'all'))==1; %quick fix for matrix inversion issue
%     Trange90=Trange;
% end

%For imposed tidal attenuation:
%dissipation_factor=0.1;%Trange remaining by end %0.25 for tst58, can be as low as 0.1
...
%-Instantaneous transport tide (->>>Trangeo), also use Ttide (the isntaneosu value)
%Trange=Trange_o;%for the time being use this one
%-90th tidal range   -Calculate Trange90 as a function of Trange90_o and Ttide90
%Trange90=Trange90_o;%for the time being use this one
%MSL90 remains constant in space!!!
%REPAD EVERYTHING AS A MATRIX, NOT A 1D transect

%upper limit for vegetation range
%dBup=0.7;%MSL90+Trange90/2+0.1;
...
minponddepth=min(minponddepth,Trange/2);
...
%dtide(S==1)=max(0, mean(Trange/2,'all')-(z(S==1)-msl)-max(0,DIF(S==1)-pondleakage));%original version, not accurate
dtide(S==1)=max(0, Trange(S==1)/2 -(z(S==1)-msl)-max(0,DIF(S==1)-pondleakage));
...
%P.UlimCURV=0.05;P.hlimCURV=0.2;
%MASKCURVATURE=(S==0 & ((U>UlimCURV & h>hlimCURV)  |  VEG==0));
MASKCURVATURE=(VEG==0);

Uo=sqrt(Ux.^2+Uy.^2);
Uxo=Ux./Uo;Uyo=Uy./Uo;%Uxo=Ux;Uyo=Uy;
Uxo(MASKCURVATURE==0)=NaN;Uyo(MASKCURVATURE==0)=NaN;

%normal velocity vector
Unx=-Uy;Uny=Ux;

cur=curlNAN(Uyo,Uxo)/dx;
cur=cur.*U;
%cur=cur*0.2;
...
%smooth the transverse flow
%a_smoothtransverseflow=1;
%Unx=diffusetransverseflow(isfinite(cur),Unx,a_smoothtransverseflow,dx,U);
%Uny=diffusetransverseflow(isfinite(cur),Uny,a_smoothtransverseflow,dx,U);

%cur(isnan(cur))=0;%You no longer use cur, so you don't need to remove the %NaNs
%Unx(isnan(Unx))=0;Uny(isnan(Uny))=0;

UCx=modifyflowcurvatureBASIC(A==1,(MASKCURVATURE==1).*Ux,advectflow,dx,Unx,Uny,MASKCURVATURE);
UCy=modifyflowcurvatureBASIC(A==1,(MASKCURVATURE==1).*Uy,advectflow,dx,Unx,Uny,MASKCURVATURE);
UC=sqrt(UCx.^2+UCy.^2);

deltaUC=UC*dx.*(MASKCURVATURE==0);

%figure
% subplot(2,1,1);
% imagesc(U');caxis([0 0.4]);colormap('jet')
%Uo=U;

%these were original
% Ux(MASKCURVATURE==1)=UCx(MASKCURVATURE==1);
% Uy(MASKCURVATURE==1)=UCy(MASKCURVATURE==1);
%this turns off "bank pull" leaves on "bar push"
Ux(MASKCURVATURE==1 & abs(Ux)>abs(UCx))=UCx(MASKCURVATURE==1 & abs(Ux)>abs(UCx));
Uy(MASKCURVATURE==1 & abs(Uy)>abs(UCy))=UCy(MASKCURVATURE==1 & abs(Uy)>abs(UCy));

U=sqrt(Ux.^2+Uy.^2);

% subplot(2,1,2);
%imagesc((U-Uo)');caxis([-0.1 0.1]);colormap('jet')
% figure
% imagesc(Unx')
% pause
...
%STOCAIST BANK EROSION%
MASK=MASKCURVATURE;
[deltaY1,deltaY2,deltaY3,Pedge,Y2OX,EdgeERY1,EdgeERY2,EdgeERY3]=EdgeerosionCURVEDFLOW(deltaUC,z,a_bankerosion,999,fox,dt,dx,MASK,A,fracY1,fracY2,fracY3,Y2OX);
Y1=Y1-deltaY1;Y2=Y2-deltaY2;Y3=Y3-deltaY3;%erode the mardsh edge fully
EDGESED=diffuseedgesediments((A==1),EdgeERY2,10*ho,dx); %put eroded sediment only in chnannels, not on marsh
Y2=Y2+EDGESED;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
MASK=0*A+1;
MASK(ho<=hwSea_lim | A==0 | VEG==1 | Zlev>=dBlo)=0;  %| Zlev>=dBlo REMOVED
...
if (computetide==1 | computeriver==1)
    ...
    if computemud==1
            ...
            if ebbfloodcorrection==1
                [E2E,E2tideE]=totalsedimenterosionMUDsine(Vebb,MANN,VEG,fTide,UR,Uwave_sea,Uwave4mud,Tp_sea,Tp_swell,fMFswell,fMFsea,fMFriver,taucr,tcrgradeint,leveltauincrease,taucrVEG,me,h,Zlev,NaN,computeSeaWaves,computeSwellwave,computeriver);
                [E2F,E2tideF]=totalsedimenterosionMUDsine(Vflood,MANN,VEG,fTide,UR,Uwave_sea,Uwave4mud,Tp_sea,Tp_swell,fMFswell,fMFsea,fMFriver,taucr,tcrgradeint,leveltauincrease,taucrVEG,me,h,Zlev,NaN,computeSeaWaves,computeSwellwave,computeriver);
                E2=(E2E+E2F)/2;E2tide=(E2tideE+E2tideF)/2;
            else
                [E2,E2tide]=totalsedimenterosionMUDsine(U,MANN,VEG,fTide,UR,Uwave_sea,Uwave4mud,Tp_sea,Tp_swell,fMFswell,fMFsea,fMFriver,taucr,tcrgradeint,leveltauincrease,taucrVEG,me,h,Zlev,NaN,computeSeaWaves,computeSwellwave,computeriver);
            end      
    ...
    flagSANDMUD=0;q2=NaN;qm2=NaN;qN=NaN;qmN=NaN; %just to pass through function from newer version of sedtran
    %Advection-Diffusion Sediment transport
    if computesand==1;
        WS=A*0+ws1;
        [EmD1,SSM,FLX1,XXTide,XXRiver]=sedtran_trackflux3(flagSANDMUD,h,A,SPCLcell,0,DiffSsand,h,ho,E1,WS,dx,dt,rbulk1,co1,Ux,Uy,fTide,Ttide,q1,qm1,qN,qmN,periodic,computeriver,computetidalcurrent,residualcurrents,kro,co1mouth,FLX1,tracksedimentfluxes,XX);  
    ...
    if computemud==1;
        ...
        %[EmD2new,SSMnew,FLX2new,XXTide,XXRiver]=sedtran_trackflux(flagSANDMUD,h,A,SPCLcell,0,DiffSsand,h,ho,E2,WS,dx,dt,rbulk2,co2,Ux,Uy,fTide,Ttide,q2,qm2,qN,qmN,periodic,computeriver,computetidalcurrent,residualcurrents,kro,co2mouth,FLX2,tracksedimentfluxes,XX);%messed_up_somehow 
        %[EmD2,SSM,FLX2]=sedtran(1,h,A,SPCLcell,DoMUD,DiffSmud,h,ho,E2,WS,dx,dt,rbulk2,co2,Ux,Uy,FLX2,fTide,Ttide,URx,URy,UresX,UresY,periodic,computeriver,computetide,residualcurrents,kro,1,co2mouth*fMFriver,QsmouthMUD);%ogFunction
        [EmD2,SSM,FLX2,XXTide,XXRiver]=sedtran_trackflux3(1,h,A,SPCLcell,DoMUD,DiffSmud,h,ho,E2,WS,dx,dt,rbulk2,co2,Ux,Uy,FLX2,fTide,Ttide,URx,URy,UresX,UresY,periodic,computeriver,computetide,residualcurrents,kro,1,co2mouth*fMFriver,tracksedimentfluxes,QsmouthMUD,XX);  %i think this one works 
...
%%%compaction%%%
Zlev=z-msl;
MARSH=Zlev>dBlo & Zlev<dBup;;
compactVOL=compact*(MARSH==1)*dt;
Y2=Y2-compactVOL;
Y2compact=Y2compact+sum(compactVOL,'all');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
znew=bedcreepponds(z,A,Active,Yreduction,crMUD,crMARSH,crbank,dx,dt,VEG,S,Qs2,rbulk2,alphaMUD,facQsbank);%,deltaUC,a_bankcreep);  %MUD CREEP  MARSH
deltaY2=z-znew;
