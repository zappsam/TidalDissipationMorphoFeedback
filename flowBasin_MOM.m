function [U,Ux,Uy,Umx,Umy]=flowBasin(A,MANN,h,d,dx,DH,T,periodic,kro);
PARU=[1,1,1];PARS=1;
methodP=PARU(1);methodU=PARU(2);methodI=PARU(3);
...
%the translated cell
if periodic==0;[a,q]=excludeboundarycell(k,N,M,p);
elseif periodic==1;[a,q]=periodicY(k,N,M,p);end %for the long-shore

a=a(A(q(a))>0);%exlcude the translated cell that are NOLAND cells
%DD=(D(p(a))+D(q(a)))/2;
DD=min(D(p(a)),D(q(a)));%.*(fM(p(a))+fM(q(a)))/2; MEGLIO
%DD=max(D(p(a)),D(q(a)));%.*(fM(p(a))+fM(q(a)))/2; 
...
Ux=max(abs(U1),abs(Um1)).*sign(U1+Um1); %MEGLIO
...
Umx=(U1+Um1)/2;
Umy=(UN+UmN)/2;
%Umx=max(abs(U1),abs(Um1)).*sign(U1+Um1); %MEGLIO
